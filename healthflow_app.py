...
if page == "🏥 ED Status":
    ...
    with col2:
        county_filter = st.selectbox("", ["All Counties"]+list(HOSPITAL_MAP.keys()), label_visibility="collapsed")
    with col3:
        if st.button("⚙️ Get recommendation", use_container_width=True):
            st.session_state["goto_advice"] = True
            st.rerun()

    if st.session_state.get("goto_advice"):
        st.session_state["goto_advice"] = False

    flat = [(c,h) for c,hs in HOSPITAL_MAP.items() for h in hs]
    if county_filter != "All Counties":
        flat = [(c,h) for c,h in flat if c==county_filter]
    if search:
        flat = [(c,h) for c,h in flat if search.lower() in h.lower() or search.lower() in c.lower()]

    st.markdown(f"<div style='color:#64748B;font-size:0.85rem;margin-bottom:12px'>Showing <strong>{len(flat)}</strong> of <strong>{sum(len(v) for v in HOSPITAL_MAP.values())}</strong> hospitals</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("📍 Step 1 — Your location & insurance", expanded=True):
        ...
        with col1:
            county_pa = st.selectbox("County / City", list(HOSPITAL_MAP.keys()), key="pa_c")
